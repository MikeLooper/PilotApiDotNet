...
{
	/// <summary>
	/// Integration tests that exercise the real <see cref="OrderDetailsRepository"/>, real <see cref="SqlBuilder"/>,
	...
	/// The composite key is ordered (ProductID, OrderID), matching <see cref="OrderDetailsRepository"/>'s
	/// <c>KeyColumnNames</c> definition.
	/// </remarks>
	[TestFixture]
	public class OrderDetailsRepositoryIntegrationTests
	{
		[Test]
		public async Task DeleteAsync_WhenNoRowsAffected_RollsBackRealTransaction_AndReturnsError_Test()
		{
			...
			// Act
			var response = await repository.DeleteAsync(new[] { 3, 10248 });
		...
		public async Task GetAllAsync_ReturnsMappedEntities_UsingRealSqlBuilderGeneratedQuery_Test()
		{
			...
			Assert.That(response.Result![0].UnitPrice, Is.EqualTo(14.0m));

			var executedCommand = fakeConnection.ExecutedCommands.Single();
			Assert.That(executedCommand.CommandText, Does.Contain("SELECT"));
			Assert.That(executedCommand.CommandText, Does.Contain("Order Details"));
		}
		...
		public async Task GetAsync_BuildsRealParameterizedQuery_AndCommitsTransaction_Test()
		{
			...
			table.Columns.Add(nameof(OrderDetailsEntity.OrderID), typeof(int));
			table.Columns.Add(nameof(OrderDetailsEntity.UnitPrice), typeof(decimal));
			table.Rows.Add(5, 10249, 42.4m);
			fakeConnection.NextQueryResult = table;

			// Act
			var response = await repository.GetAsync(new[] { 5, 10249 });

			// Assert
			Assert.That(response, Is.Not.Null);
			Assert.That(response!.Result, Is.Not.Null);
			Assert.That(response.Result!.UnitPrice, Is.EqualTo(42.4m));

			var executedCommand = fakeConnection.ExecutedCommands.Single();
			Assert.That(executedCommand.CommandText, Does.Contain("WHERE"));
			Assert.That(executedCommand.Parameters, Does.ContainKey("ProductID"));
			...
			Assert.That(executedCommand.Parameters["OrderID"], Is.EqualTo(10249));
		}
