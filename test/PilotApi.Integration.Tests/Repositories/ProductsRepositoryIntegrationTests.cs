...
{
	/// <summary>
	/// Integration tests that exercise the real <see cref="ProductsRepository"/>, real <see cref="SqlBuilder"/>,
	...
	public class ProductsRepositoryIntegrationTests
	{
		...
		public async Task GetAllAsync_ReturnsMappedEntities_UsingRealSqlBuilderGeneratedQuery_Test()
		{
			...
			Assert.That(response.Result![0].ProductName, Is.EqualTo("Chai"));

			var executedCommand = fakeConnection.ExecutedCommands.Single();
			Assert.That(executedCommand.CommandText, Does.Contain("SELECT"));
			Assert.That(executedCommand.CommandText, Does.Contain("Products"));
		}
		...
		public async Task GetAsync_BuildsRealParameterizedQuery_AndCommitsTransaction_Test()
		{
			...
			table.Columns.Add(nameof(ProductsEntity.ProductID), typeof(int));
			table.Columns.Add(nameof(ProductsEntity.ProductName), typeof(string));
			table.Rows.Add(5, "Chef Anton's Gumbo Mix");
			fakeConnection.NextQueryResult = table;
			...
			Assert.That(response.Result!.ProductName, Is.EqualTo("Chef Anton's Gumbo Mix"));
