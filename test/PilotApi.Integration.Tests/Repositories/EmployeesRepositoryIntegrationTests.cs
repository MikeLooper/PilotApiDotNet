...
{
	/// <summary>
	/// Integration tests that exercise the real <see cref="EmployeesRepository"/>, real <see cref="SqlBuilder"/>,
	...
	public class EmployeesRepositoryIntegrationTests
	{
		...
		public async Task GetAllAsync_ReturnsMappedEntities_UsingRealSqlBuilderGeneratedQuery_Test()
		{
			...
			Assert.That(response.Result![0].LastName, Is.EqualTo("Davolio"));

			var executedCommand = fakeConnection.ExecutedCommands.Single();
			Assert.That(executedCommand.CommandText, Does.Contain("SELECT"));
			Assert.That(executedCommand.CommandText, Does.Contain("Employees"));
		}
		...
		public async Task GetAsync_BuildsRealParameterizedQuery_AndCommitsTransaction_Test()
		{
			...
			Assert.That(response.Result!.LastName, Is.EqualTo("Buchanan"));

			var executedCommand = fakeConnection.ExecutedCommands.Single();
			Assert.That(executedCommand.CommandText, Does.Contain("WHERE"));
			Assert.That(executedCommand.Parameters, Does.ContainKey("EmployeeID"));
			Assert.That(executedCommand.Parameters["EmployeeID"], Is.EqualTo(5));
		}
