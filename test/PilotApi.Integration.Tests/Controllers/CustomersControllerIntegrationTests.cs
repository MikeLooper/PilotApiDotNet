...
{
	/// <summary>
	/// Integration tests that exercise the real <see cref="CustomersController"/>, real <see cref="CustomersService"/>,
	/// real <see cref="DataMapperHandler"/>, and the real <see cref="PilotApi.Repositories.Repositories.CustomersRepository"/>
	/// (built via <see cref="IntegrationTestDoublesUtilities.GetCustomersService"/>) all wired together. Only the raw
	...
	public class CustomersControllerIntegrationTests
	{
		...
		public async Task Add_WhenRepositoryInsertSucceeds_ReturnsCreatedAtActionWithNewId_Test()
		{
			...
			table.Rows.Add("ALFKI");
			fakeConnection.NextQueryResult = table;

			var model = new CustomersDto { CompanyName = "Alfreds Futterkiste" };
			...
			var addResponse = createdResult.Value as AddResponseString;
			Assert.That(addResponse, Is.Not.Null);
			Assert.That(addResponse!.Id, Is.EqualTo("ALFKI"));
		}
		...
		public async Task Delete_WhenRepositoryReturnsError_ReturnsBadRequestWithWarningHeader_Test()
		{
			...
			// Act
			var actionResult = await controller.Delete("ALFKI", CancellationToken.None);
		...
		public async Task GetAll_WhenRepositoryReturnsEntities_ReturnsOkWithMappedDtos_Test()
		{
			...
			var dtos = okResult!.Value as IReadOnlyList<CustomersDto>;
			Assert.That(dtos, Is.Not.Null);
			Assert.That(dtos!, Has.Count.EqualTo(2));
			Assert.That(dtos![0].CompanyName, Is.EqualTo("Alfreds Futterkiste"));
		}
		...
		public async Task GetById_WhenRepositoryReturnsEntity_ReturnsOkWithMappedDto_Test()
		{
			...
			// Act
			var actionResult = await controller.GetById("ALFKI", CancellationToken.None);

			// Assert
			var okResult = actionResult as OkObjectResult;
			Assert.That(okResult, Is.Not.Null);
			var dto = okResult!.Value as CustomersDto;
			Assert.That(dto, Is.Not.Null);
			Assert.That(dto!.CompanyName, Is.EqualTo("Alfreds Futterkiste"));
		}
		...
		public async Task GetById_WhenRepositoryReturnsNull_ReturnsNotFound_Test()
		{
			...
			// Act
			var actionResult = await controller.GetById("NOPE1", CancellationToken.None);
