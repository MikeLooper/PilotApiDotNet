...
{
	/// <summary>
	/// Integration tests that exercise the real <see cref="OrdersController"/>, real <see cref="OrdersService"/>,
	/// real <see cref="DataMapperHandler"/>, and the real <see cref="PilotApi.Repositories.Repositories.OrdersRepository"/>
	/// (built via <see cref="IntegrationTestDoublesUtilities.GetOrdersService"/>) all wired together. Only the raw
	...
	public class OrdersControllerIntegrationTests
	{
		...
		public async Task Add_WhenRepositoryInsertSucceeds_ReturnsCreatedAtActionWithNewId_Test()
		{
			...
			var service = IntegrationTestDoublesUtilities.GetOrdersService(fakeConnection);
			using var controller = new OrdersController(service);

			var table = new DataTable();
			table.Columns.Add("Id", typeof(int));
			table.Rows.Add(10248);
			fakeConnection.NextQueryResult = table;

			var model = new OrdersDto { CustomerID = "VINET" };
			...
			Assert.That(addResponse!.Id, Is.EqualTo(10248));
		}
		...
		public async Task GetAll_WhenRepositoryReturnsEntities_ReturnsOkWithMappedDtos_Test()
		{
			...
			var dtos = okResult!.Value as IReadOnlyList<OrdersDto>;
			Assert.That(dtos, Is.Not.Null);
			Assert.That(dtos!, Has.Count.EqualTo(2));
			Assert.That(dtos![0].CustomerID, Is.EqualTo("VINET"));
		}
		...
		public async Task GetById_WhenRepositoryReturnsEntity_ReturnsOkWithMappedDto_Test()
		{
			...
			// Act
			var actionResult = await controller.GetById(10250, CancellationToken.None);

			// Assert
			var okResult = actionResult as OkObjectResult;
			Assert.That(okResult, Is.Not.Null);
			var dto = okResult!.Value as OrdersDto;
			Assert.That(dto, Is.Not.Null);
			Assert.That(dto!.CustomerID, Is.EqualTo("HANAR"));
		}
