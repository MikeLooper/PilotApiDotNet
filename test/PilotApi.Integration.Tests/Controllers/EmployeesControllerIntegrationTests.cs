...
{
	/// <summary>
	/// Integration tests that exercise the real <see cref="EmployeesController"/>, real <see cref="EmployeesService"/>,
	/// real <see cref="DataMapperHandler"/>, and the real <see cref="PilotApi.Repositories.Repositories.EmployeesRepository"/>
	/// (built via <see cref="IntegrationTestDoublesUtilities.GetEmployeesService"/>) all wired together. Only the raw
	...
	public class EmployeesControllerIntegrationTests
	{
		...
		public async Task Add_WhenRepositoryInsertSucceeds_ReturnsCreatedAtActionWithNewId_Test()
		{
			...
			var service = IntegrationTestDoublesUtilities.GetEmployeesService(fakeConnection);
			using var controller = new EmployeesController(service);
			...
			var model = new EmployeesDto { LastName = "Davolio" };
		...
		public async Task GetAll_WhenRepositoryReturnsEntities_ReturnsOkWithMappedDtos_Test()
		{
			...
			table.Rows.Add(1, "Davolio");
			table.Rows.Add(2, "Fuller");
			fakeConnection.NextQueryResult = table;
			...
			var dtos = okResult!.Value as IReadOnlyList<EmployeesDto>;
			Assert.That(dtos, Is.Not.Null);
			Assert.That(dtos!, Has.Count.EqualTo(2));
			Assert.That(dtos![0].LastName, Is.EqualTo("Davolio"));
		}
		...
		public async Task GetById_WhenRepositoryReturnsEntity_ReturnsOkWithMappedDto_Test()
		{
			...
			var okResult = actionResult as OkObjectResult;
			Assert.That(okResult, Is.Not.Null);
			var dto = okResult!.Value as EmployeesDto;
			Assert.That(dto, Is.Not.Null);
			Assert.That(dto!.LastName, Is.EqualTo("King"));
		}
