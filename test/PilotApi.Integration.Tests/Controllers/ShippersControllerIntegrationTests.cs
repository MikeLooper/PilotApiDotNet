...
{
	/// <summary>
	/// Integration tests that exercise the real <see cref="ShippersController"/>, real <see cref="ShippersService"/>,
	/// real <see cref="DataMapperHandler"/>, and the real <see cref="PilotApi.Repositories.Repositories.ShippersRepository"/>
	/// (built via <see cref="IntegrationTestDoublesUtilities.GetShippersService"/>) all wired together. Only the raw
	...
	/// </remarks>
	[TestFixture]
	public class ShippersControllerIntegrationTests
	{
		[Test]
		public async Task Add_WhenRepositoryInsertSucceeds_ReturnsCreatedAtActionWithNewId_Test()
		{
			...
			var service = IntegrationTestDoublesUtilities.GetShippersService(fakeConnection);
			using var controller = new ShippersController(service);
			...
			var model = new ShippersDto { CompanyName = "Speedy Express" };
		...
		public async Task GetAll_WhenRepositoryReturnsEntities_ReturnsOkWithMappedDtos_Test()
		{
			...
			var dtos = okResult!.Value as IReadOnlyList<ShippersDto>;
			Assert.That(dtos, Is.Not.Null);
			Assert.That(dtos!, Has.Count.EqualTo(2));
			Assert.That(dtos![0].CompanyName, Is.EqualTo("Speedy Express"));
		}
		...
		public async Task GetById_WhenRepositoryReturnsEntity_ReturnsOkWithMappedDto_Test()
		{
			...
			// Act
			var actionResult = await controller.GetById(3, CancellationToken.None);

			// Assert
			var okResult = actionResult as OkObjectResult;
			Assert.That(okResult, Is.Not.Null);
			var dto = okResult!.Value as ShippersDto;
			Assert.That(dto, Is.Not.Null);
			Assert.That(dto!.CompanyName, Is.EqualTo("Federal Shipping"));
		}
