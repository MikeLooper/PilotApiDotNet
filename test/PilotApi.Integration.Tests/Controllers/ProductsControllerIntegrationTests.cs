...
{
	/// <summary>
	/// Integration tests that exercise the real <see cref="ProductsController"/>, real <see cref="ProductsService"/>,
	/// real <see cref="DataMapperHandler"/>, and the real <see cref="PilotApi.Repositories.Repositories.ProductsRepository"/>
	/// (built via <see cref="IntegrationTestDoublesUtilities.GetProductsService"/>) all wired together. Only the raw
	...
	public class ProductsControllerIntegrationTests
	{
		...
		public async Task Add_WhenRepositoryInsertSucceeds_ReturnsCreatedAtActionWithNewId_Test()
		{
			...
			var service = IntegrationTestDoublesUtilities.GetProductsService(fakeConnection);
			using var controller = new ProductsController(service);
			...
			var model = new ProductsDto { ProductName = "Chai" };
		...
		public async Task GetAll_WhenRepositoryReturnsEntities_ReturnsOkWithMappedDtos_Test()
		{
			...
			var dtos = okResult!.Value as IReadOnlyList<ProductsDto>;
			Assert.That(dtos, Is.Not.Null);
			Assert.That(dtos!, Has.Count.EqualTo(2));
			Assert.That(dtos![0].ProductName, Is.EqualTo("Chai"));
		}
		...
		public async Task GetById_WhenRepositoryReturnsEntity_ReturnsOkWithMappedDto_Test()
		{
			...
			table.Columns.Add(nameof(ProductsEntity.ProductID), typeof(int));
			table.Columns.Add(nameof(ProductsEntity.ProductName), typeof(string));
			table.Rows.Add(7, "Uncle Bob's Organic Dried Pears");
			fakeConnection.NextQueryResult = table;
			...
			var okResult = actionResult as OkObjectResult;
			Assert.That(okResult, Is.Not.Null);
			var dto = okResult!.Value as ProductsDto;
			Assert.That(dto, Is.Not.Null);
			Assert.That(dto!.ProductName, Is.EqualTo("Uncle Bob's Organic Dried Pears"));
		}
